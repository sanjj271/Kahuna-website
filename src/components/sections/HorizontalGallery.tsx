    ...
    >
      ...
      <div className="flex gap-4 md:gap-8 px-4 md:px-8 overflow-x-auto snap-x snap-mandatory no-scrollbar pb-12 cursor-grab active:cursor-grabbing">
        {siteContent.gallery.map((src, index) => (
          <div
            key={index}
            className={`relative h-[40vh] md:h-[50vh] min-w-[85vw] md:min-w-[40vw] shrink-0 snap-center overflow-hidden rounded-xl border border-border ${index % 2 === 0 ? "mt-0 md:mt-16" : "mt-8 md:mt-0"
              }`}
          >
            <Image
              src={src}
